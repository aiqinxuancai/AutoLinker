...
description: >-
  ...
  dynamic/static ec auto-swap, the local MCP HTTP server and its tool set,
  ...
  configuring, or driving AutoLinker, or connecting an external AI agent to an
  易语言 IDE through it.
---
...
outside the IDE. AutoLinker works *inside* the IDE process, so all reads/writes
go through the IDE's in-memory project, not the raw file.
