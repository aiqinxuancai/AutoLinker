...
constexpr auto kSilentCompileMessageBoxGuardTimeout = std::chrono::minutes(10);
...
bool IsSilentCompileOutputPathMessageBoxGuardActive()
{
	std::lock_guard<std::mutex> lock(g_silentCompileOutputPathMutex);
...
int GetSilentCompileMessageBoxAutoResponse(UINT type)
{
...
std::vector<CommandLineToken> SplitCommandLineTokens(const std::string& commandLine)
{
	std::vector<CommandLineToken> tokens;
	size_t pos = 0;
	while (pos < commandLine.size()) {
		while (pos < commandLine.size() && std::isspace(static_cast<unsigned char>(commandLine[pos]))) {
			++pos;
		}
		...
		std::string raw = commandLine.substr(begin, pos - begin);
		tokens.push_back(CommandLineToken{ raw, RemoveDoubleQuotes(raw) });
	}
	return tokens;
}

std::string JoinCommandLineTokens(const std::vector<CommandLineToken>& tokens)
{
	std::string out;
...
std::string GetLowerFileName(const std::string& pathText)
{
	try {
		return ToLowerAsciiLocal(std::filesystem::path(pathText).filename().string());
	}
	catch (...) {
		return ToLowerAsciiLocal(pathText);
	}
}

std::string GetLowerExtension(const std::string& pathText)
{
	try {
		return ToLowerAsciiLocal(std::filesystem::path(pathText).extension().string());
	}
	catch (...) {
		return std::string();
	}
...
bool IsAbsoluteLibInputToken(const std::string& tokenValue)
{
	if (tokenValue.empty() || tokenValue.front() == '/' || tokenValue.front() == '-') {
		return false;
	}
	if (GetLowerExtension(tokenValue) != ".lib") {
		return false;
	}
	try {
		return std::filesystem::path(tokenValue).is_absolute();
	}
	catch (...) {
		return false;
	}
...
bool FileNameMatchesAnyForceLib(const std::string& tokenValue, const std::vector<std::string>& forceLibPaths)
{
	const std::string tokenFileName = GetLowerFileName(tokenValue);
	if (tokenFileName.empty()) {
		return false;
	}
	for (const std::string& forceLibPath : forceLibPaths) {
		if (tokenFileName == GetLowerFileName(forceLibPath)) {
			return true;
		}
	}
	return false;
...
bool IsExistingPathLocal(const std::string& pathText)
{
	std::error_code ec;
	const bool exists = std::filesystem::exists(pathText, ec);
	return !ec && exists;
}

bool IsSamePathTextLocal(const std::string& left, const std::string& right)
{
	...
	try {
		std::filesystem::path leftPath(left);
		std::filesystem::path rightPath(right);
		leftPath = leftPath.lexically_normal();
		rightPath = rightPath.lexically_normal();
		return ToLowerAsciiLocal(leftPath.string()) == ToLowerAsciiLocal(rightPath.string());
	}
	catch (...) {
		return ToLowerAsciiLocal(left) == ToLowerAsciiLocal(right);
	}
}

std::string RemoveOriginalAbsoluteLibInputsByFileName(
	const std::string& commandLine,
	const std::vector<std::string>& forceLibPaths,
	const std::string& protectedLibPath)
{
	if (commandLine.empty() || forceLibPaths.empty()) {
		return commandLine;
	}

	std::vector<CommandLineToken> filteredTokens;
	for (const auto& token : SplitCommandLineTokens(commandLine)) {
		if (!IsAbsoluteLibInputToken(token.value) || !FileNameMatchesAnyForceLib(token.value, forceLibPaths)) {
			filteredTokens.push_back(token);
			continue;
		}

		// krnln_static.lib 是后续插入强制链接库的锚点，任何情况下都不能被同名清理规则移除。
		if (IsSamePathTextLocal(token.value, protectedLibPath)) {
			filteredTokens.push_back(token);
			continue;
		}

		// 只清理 IDE 生成的“同名但已不存在”的绝对路径输入项，避免误删用户确实需要的同名有效库。
		if (!IsExistingPathLocal(token.value)) {
			OutputStringToELog(std::format("remove missing duplicated IDE lib input: {}", token.value));
			continue;
		}
...
bool HasForceSwitch(const std::string& commandLine)
{
	for (const auto& token : SplitCommandLineTokens(commandLine)) {
		const std::string value = ToLowerAsciiLocal(token.value);
		if (value == "/force" || value.rfind("/force:", 0) == 0 ||
			value == "-force" || value.rfind("-force:", 0) == 0) {
			return true;
		}
	}
	return false;
	...
	LPPROCESS_INFORMATION lpProcessInformation)
{
	...
	if (!krnlnPath.empty()) {
		...
		if (!forceLinkRules.empty()) {
			...
			const bool appendForce = forceLinkLibManager.getAppendForce();
			OutputStringToELog(std::format("current linker: {}", currentLinkerName));

			std::string libCmd;
			std::vector<std::string> matchedLibPaths;
				...
				if (!linkerName.empty()) {
					if (currentLinkerName.find(linkerName) != std::string::npos) {
						if (std::filesystem::exists(libPath)) {
							...
							libCmd += "\"" + libPath + "\"";
							matchedLibPaths.push_back(libPath);
						}
					}
					else {
						OutputStringToELog(std::format(
			...
			if (!matchedLibPaths.empty()) {
				commandLine = RemoveOriginalAbsoluteLibInputsByFileName(commandLine, matchedLibPaths, krnlnPath);
				std::string newLibs = libCmd + " \"" + krnlnPath + "\"";
				if (appendForce && !HasForceSwitch(commandLine)) {
					newLibs += " /FORCE";
				}
...
int WINAPI MyMessageBoxA(HWND hWnd, LPCSTR lpText, LPCSTR lpCaption, UINT uType)
{
	...
	const bool silentCompileGuardActive = IsSilentCompileOutputPathMessageBoxGuardActive();
	if (silentCompileGuardActive && IsSilentCompileModulePublishPrompt(text)) {
		OutputStringToELog("auto-confirm ecom publish prompt during silent compile");
		return IDYES;
	}
	if (silentCompileGuardActive) {
		OutputStringToELog(std::format(
			"[SilentCompile] suppressed MessageBoxA caption={} text={}",
			caption,
			text));
		return GetSilentCompileMessageBoxAutoResponse(uType);
	}
...
int WINAPI MyMessageBoxW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType)
{
	...
	if (IsSilentCompileOutputPathMessageBoxGuardActive()) {
		OutputStringToELog("[SilentCompile] suppressed MessageBoxW during silent compile");
		return GetSilentCompileMessageBoxAutoResponse(uType);
	}
	if (HeadlessCompileRunner::HasHeadlessCompileRequest()) {
