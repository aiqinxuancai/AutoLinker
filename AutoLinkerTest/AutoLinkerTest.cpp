...
void PrintUsage()
{
	...
	std::cout << "  AutoLinkerTest ai-endpoint-config-self-test" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	if (commandName == "goal-mode-self-test") {
		if (argc != 2) {
			PrintUsage();
			return EXIT_FAILURE;
		}
		std::vector<char> buffer(16384, '\0');
		const int result = AutoLinkerTest_RunGoalModeSelfTest(
			buffer.data(),
			static_cast<int>(buffer.size()));
		if (result < 0) {
			return PrintStringResult(commandName.c_str(), result, buffer.data());
	...
	if (commandName == "ai-endpoint-config-self-test") {
		if (argc != 2) {
			PrintUsage();
			return EXIT_FAILURE;
		}
		std::vector<char> buffer(65536, '\0');
		const int result = AutoLinkerTest_RunAIEndpointConfigSelfTest(
			buffer.data(),
			static_cast<int>(buffer.size()));
		if (result < 0) {
			return PrintStringResult(commandName.c_str(), result, buffer.data());
