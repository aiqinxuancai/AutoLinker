...
“每次MCP写入或修改代码后自动保存代码”默认关闭。启用后，MCP 写工具成功修改当前工程时会调用 IDE 保存；仅当当前 `.e` / `.ec` 已有磁盘路径且文件存在时生效。
...
- 安装技能不会执行其中的脚本。AI 后续如需运行本机命令，会在 AI 对话内按写入操作相同的允许逻辑确认；开启自动允许后直接执行。
