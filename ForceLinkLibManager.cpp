...
bool ForceLinkLibManager::getAppendForce() const
{
	return appendForce_;
}

bool ForceLinkLibManager::replaceRules(const std::vector<ForceLinkLibRule>& rules, bool appendForce, std::string* errorMessage)
{
	rules_.clear();
	appendForce_ = appendForce;
...
void ForceLinkLibManager::loadConfig()
{
	...
	while (lineBegin <= text.size()) {
		...
		if (line.empty() || line[0] == ';' || line[0] == '#') {
			...
			else if (sectionName.rfind("rule", 0) == 0) {
				section = ConfigSection::Rule;
			}
			else {
			...
			if (pos != std::string::npos) {
				...
				if (key == "appendforce") {
					appendForce_ = ParseBoolValue(value, false);
				}
			}
		}
		else if (section == ConfigSection::Rule) {
			const size_t pos = line.find('=');
			if (pos != std::string::npos) {
				const std::string key = ToLowerAscii(TrimAscii(line.substr(0, pos)));
				const std::string value = TrimAscii(line.substr(pos + 1));
...
bool ForceLinkLibManager::saveConfig(std::string* errorMessage) const
{
	...
	text += "; appendForce 控制是否在命令中附加 /FORCE，默认关闭。\r\n";
	text += "\r\n[settings]\r\n";
	text += "appendForce=";
	text += appendForce_ ? "1" : "0";
	text += "\r\n";
