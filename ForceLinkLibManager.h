...
public:
	...
	// 返回是否在重写链接命令时附加 /FORCE，默认关闭。
	bool getAppendForce() const;

	// 替换全部规则并保存到 ForceLinkLib.ini。
	bool replaceRules(const std::vector<ForceLinkLibRule>& rules, bool appendForce, std::string* errorMessage = nullptr);
