...
- 支持本地 MCP 工具调用，可用统一的文件路径方式列出、搜索、读取、编辑当前工程源码。
- 当前工程源码读写基于 e-packager 解包镜像：读取/搜索只面对解包目录，写回仍走 IDE 真实页整页复制/粘贴逻辑，成功后自动让镜像失效并在下次读取时重建。
...
AutoLinker 挂接了 IDE 的“开始调试 / 开始编译”入口：触发时遍历当前工程已导入的 ec 模块，按文件名匹配配置表，命中后就地把该模块替换为对应版本（移除旧的、导入新的），切换过程会输出到 E 调试输出窗口，例如：

```text
切换模块:...\VMPSDK_LIB.ec -> ...\VMPSDK.ec
```

支持版本 `5.71`、`5.95`（其他版本未测试，但原理通用，应当可用）。

**使用方法**

在易语言 IDE 的「工具」菜单中打开 **AutoLinker EC模块自动切换设置**，在设置页中维护切换规则：

1. 每条规则填写一对模块：**动态调试版**（调试时使用）与**静态编译版**（编译时使用）。
2. 只需填模块**文件名**即可，无需完整路径；省略 `.ec` 后缀时会自动补全。
3. 点击保存即生效，下次调试 / 编译时自动按规则切换。

> **注意**
> - 成对的两个 ec 必须放在**同一个文件夹**中（替换时只改文件名，沿用原模块所在目录）。
> - 同一条规则的动态版与静态版不能同名，且各模块名不可重复。
> - 该设置页基于 WebView2，请确保系统已安装 WebView2 运行时。
