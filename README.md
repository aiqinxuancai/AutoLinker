...
- 内置 AI 对话和外部 MCP 会话首次读取或编辑源码前必须先调用 `refresh_workspace_mirror`；AutoLinker 会用 e-packager 将当前 IDE 工程（包含未保存改动）解包到源文件目录下的 `.temp/al_<pid>_*` 临时镜像目录，源码目录不可写时回退到系统临时目录。
