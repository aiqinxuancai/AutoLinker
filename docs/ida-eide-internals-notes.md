...
IDA 静态分析已确认，“通用型”不是普通类型表中的可查找名称，而是特殊内部类型 ID：

```text
0x80000000 = 通用型 / all
0x80000008 = 语句型 / statment
```

证据链：

- `0x4FF310` 在输入 `0x80000000` 时把三个显示字段分别设置为本地化“通用型”、`all` 和类型说明。
- `0x4D6410` 解析参数、局部变量、程序集变量等声明的类型字段。
- `0x4D6610` 调用普通类型查找函数 `0x462890`。
- “通用型”不在普通类型表中，因此查找返回 `0`。
- `0x4D663F` 至 `0x4D666A` 随后把未识别名称注册为 `0x41000000 | 临时序号` 的自定义类型，并设置未解析标志。
- 该临时类型仍会被 formatter 输出成“通用型”，但编译时无法解析，最终报告“没有找到名为‘通用型’的数据类型”。

AutoLinker 的写页实现因此在调用文本解析器期间，对 `0x462890` 安装短生命周期、当前线程限定的 Detours Hook。仅当查找名称精确等于“通用型”时返回 `0x80000000`，其余调用全部转发给 IDE 原函数。Hook 安装前会校验 e5.95 函数签名，并在任何页面删除动作之前失败返回，避免版本不匹配时先清空源码页。
