...
已经安装 AutoLinker 后，优先通过“工具菜单 → AutoLinker 设置 → 关于 → 组件更新”
...
也可以打开“工具菜单 → AutoLinker 设置 → 当前项目 AGENTS.md”，直接查看和编辑当前
已打开工程对应的规范文件。

### 4. 按源码切换链接器

通过“工具菜单 → AutoLinker 设置 → 链接器”添加和编辑多个 `link.ini` 配置，然后在
...
ExDui。通过“工具菜单 → AutoLinker 设置 → EC 模块切换”维护规则。必须先引用其中任意
一个 ec。

### 6. 项目级按配置编译

通过“工具菜单 → AutoLinker 设置 → 编译配置”为当前 `.e/.ec` 工程维护多个输出配置，再从
“主菜单 → 编译 → 按配置编译”选择配置。配置保存于源文件旁的
`<文件名>.autolinker.json`，支持 `auto`、窗口 EXE、控制台 EXE、DLL、`ecom`、静态编译和
PowerShell 前后置动作。选择后 AutoLinker 会生成当前 IDE 内存工程快照，启动独立隐藏的
`e.exe` 异步编译；菜单提交成功不代表产物已生成，应以最终日志和输出文件为准。完整字段、变量
和排错见仓库内的 `docs/project-build-menu-integration.md`。

### 7. 核心库 C++ 重写（性能、反破解、防御性免杀）

使用现代 C++（`.lib`、32 位）替换核心库函数。通过“工具菜单 → AutoLinker 设置 →
...
### 10. 统一设置窗口（“AutoLinker 设置”）

统一入口是“工具菜单 → AutoLinker 设置”。回答“某功能在哪里”或指导用户操作时，优先
...
| 编译配置 | “AutoLinker 设置 → 编译配置”：维护项目级输出目标、静态编译和前后置动作；执行入口在“主菜单 → 编译 → 按配置编译” |
...
工具菜单还包含“打开项目目录”、“打开 AutoLinker 配置目录”、“打开易语言目录”。

## AI 服务商配置

完整说明见 `CONFIG.md`。在“工具菜单 → AutoLinker 设置 → AI 接口”中配置。推荐流程：
...
实际选型可参考[易语言大模型基准评分](https://e-language-bench.apptest.dev)，以真实回包和无头编译结果为准。
...
“每次MCP写入或修改代码后自动保存代码”默认关闭；启用后仅会自动保存已有磁盘路径的当前 `.e` / `.ec` 源文件。
