...
constexpr UINT_PTR kProjectAgentsConfigWebViewInitTimerId = 0xAC06;
constexpr UINT kProjectAgentsConfigWebViewInitTimeoutMs = 12000;

struct ProjectAgentsConfigWebViewDialogContext {
...
bool IsValidUtf8Local(const std::string& text)
{
	if (text.empty()) {
		return true;
	}
...
bool IsProjectSourceExtension(const std::filesystem::path& path)
{
	const std::string ext = ToLowerAsciiLocal(path.extension().string());
	return ext == ".e" || ext == ".ec";
}

std::filesystem::path BuildProjectAgentsPath(const std::filesystem::path& sourcePath)
{
	return sourcePath.parent_path() / (sourcePath.stem().string() + ".AGENTS.md");
}

std::string NormalizeNewlinesToCrLf(std::string text)
{
...
std::string ReadProjectAgentsContentUtf8(const std::filesystem::path& agentsPath)
{
	std::ifstream in(agentsPath, std::ios::binary);
	if (!in.is_open()) {
		return std::string();
	}
	...
	if (!IsValidUtf8Local(content)) {
		content = LocalToUtf8TextLocal(content);
	}
	return content;
}

bool SaveProjectAgentsContentUtf8(
	...
	std::string& errorMessage)
{
	try {
		std::error_code ec;
		std::filesystem::create_directories(agentsPath.parent_path(), ec);
		if (ec) {
			errorMessage = "创建目录失败：" + ec.message();
			return false;
		}

		contentUtf8 = NormalizeNewlinesToCrLf(std::move(contentUtf8));
		std::ofstream out(agentsPath, std::ios::binary | std::ios::trunc);
		if (!out.is_open()) {
			errorMessage = "无法写入文件。";
			return false;
		}
		...
		if (!contentUtf8.empty()) {
			out.write(contentUtf8.data(), static_cast<std::streamsize>(contentUtf8.size()));
		}
		if (!out.good()) {
			errorMessage = "写入文件失败。";
			return false;
		...
		errorMessage = std::string("保存失败：") + e.what();
		return false;
	}
	catch (...) {
		errorMessage = "保存失败。";
...
std::string BuildProjectAgentsWebViewShellHtml()
{
	std::string html = LoadUtf8HtmlResourceText(IDR_HTML_PROJECT_AGENTS_CONFIG_DIALOG);
	if (!html.empty()) {
		return html;
	}
	return "<!doctype html><html><head><meta charset=\"utf-8\"></head><body>Project AGENTS.md config shell resource missing.</body></html>";
}

std::string BuildProjectAgentsWebViewPayloadJson(ProjectAgentsConfigWebViewDialogContext* ctx)
{
	nlohmann::json payload;
	payload["sourcePath"] = WideToUtf8Local(ctx->sourcePath.wstring());
	payload["agentsPath"] = WideToUtf8Local(ctx->agentsPath.wstring());
	payload["fileName"] = WideToUtf8Local(ctx->agentsPath.filename().wstring());
	payload["exists"] = std::filesystem::exists(ctx->agentsPath);
	payload["content"] = ctx->contentUtf8;
	return payload.dump(-1, ' ', false, nlohmann::json::error_handler_t::replace);
}

void ExecuteProjectAgentsWebViewScript(ProjectAgentsConfigWebViewDialogContext* ctx, const std::wstring& script)
{
...
void ApplyProjectAgentsWebViewData(ProjectAgentsConfigWebViewDialogContext* ctx)
{
	if (ctx == nullptr) {
		return;
	}

	const std::wstring payloadWide = Utf8ToWideLocal(BuildProjectAgentsWebViewPayloadJson(ctx));
	std::wstring script = L"window.autolinkerApplyProjectAgentsConfig(JSON.parse('";
	script += EscapeJsSingleQuotedWideLocal(payloadWide);
	script += L"'));";
	ExecuteProjectAgentsWebViewScript(ctx, script);
}

void NotifyProjectAgentsSaveResult(ProjectAgentsConfigWebViewDialogContext* ctx, bool ok, const std::string& messageLocal)
{
	if (ctx == nullptr) {
		return;
	}
	...
	result["ok"] = ok;
	result["message"] = LocalToUtf8TextLocal(messageLocal);
	if (ok) {
		result["content"] = ctx->contentUtf8;
		result["exists"] = std::filesystem::exists(ctx->agentsPath);
	}

	std::wstring script = L"window.autolinkerProjectAgentsSaveResult(JSON.parse('";
...
void HandleProjectAgentsWebViewSave(ProjectAgentsConfigWebViewDialogContext* ctx, const nlohmann::json& data)
{
	...
	std::string errorMessage;
	if (!SaveProjectAgentsContentUtf8(ctx->agentsPath, content, errorMessage)) {
		NotifyProjectAgentsSaveResult(ctx, false, errorMessage.empty() ? "保存 AGENTS.md 失败。" : errorMessage);
		return;
	}

	ctx->contentUtf8 = NormalizeNewlinesToCrLf(std::move(content));
	OutputStringToELog(std::format("[AI] 当前程序 AGENTS.md 已保存：{}", ctx->agentsPath.string()));
	NotifyProjectAgentsSaveResult(ctx, true, "已保存。后续 AI 请求会自动使用此项目规范。");
}

void LayoutProjectAgentsConfigWebViewDialog(HWND hWnd, ProjectAgentsConfigWebViewDialogContext* ctx)
{
...
HRESULT OnProjectAgentsConfigControllerCreated(HWND hWnd, HRESULT controllerResult, ICoreWebView2Controller* controller)
{
	auto* readyCtx = reinterpret_cast<ProjectAgentsConfigWebViewDialogContext*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
	if (readyCtx == nullptr || !IsWindow(hWnd)) {
		return S_OK;
	}
	if (FAILED(controllerResult) || controller == nullptr) {
		OutputStringToELog(std::format("[Project AGENTS Config][WebView2] create controller failed hr=0x{:08X}", static_cast<unsigned int>(controllerResult)));
		DestroyWindow(hWnd);
		return S_OK;
	}
	...
	readyCtx->webView->add_WebMessageReceived(
		Microsoft::WRL::Callback<ICoreWebView2WebMessageReceivedEventHandler>(
			[hWnd](ICoreWebView2*, ICoreWebView2WebMessageReceivedEventArgs* args) -> HRESULT {
				auto* messageCtx = reinterpret_cast<ProjectAgentsConfigWebViewDialogContext*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
				...
				try {
					...
					NotifyProjectAgentsSaveResult(messageCtx, false, "页面消息解析失败。");
				}
				return S_OK;
			}).Get(),
		nullptr);
	...
	readyCtx->webView->add_NavigationCompleted(
		Microsoft::WRL::Callback<ICoreWebView2NavigationCompletedEventHandler>(
			[hWnd](ICoreWebView2*, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
				auto* navCtx = reinterpret_cast<ProjectAgentsConfigWebViewDialogContext*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
				...
				if (isSuccess == TRUE) {
					...
					KillTimer(hWnd, kProjectAgentsConfigWebViewInitTimerId);
	...
	LayoutProjectAgentsConfigWebViewDialog(hWnd, readyCtx);
	const std::wstring shellHtml = Utf8ToWideLocal(BuildProjectAgentsWebViewShellHtml());
	if (shellHtml.empty()) {
		DestroyWindow(hWnd);
		return S_OK;
	}
...
void StartProjectAgentsConfigWebView(HWND hWnd, ProjectAgentsConfigWebViewDialogContext* ctx)
{
	...
	const HRESULT hr = CreateCoreWebView2EnvironmentWithOptions(
		...
		Microsoft::WRL::Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
			[hWnd](HRESULT envResult, ICoreWebView2Environment* environment) -> HRESULT {
				auto* innerCtx = reinterpret_cast<ProjectAgentsConfigWebViewDialogContext*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
				if (innerCtx == nullptr || !IsWindow(hWnd)) {
					return S_OK;
				}
				if (FAILED(envResult) || environment == nullptr) {
					OutputStringToELog(std::format("[Project AGENTS Config][WebView2] create environment failed hr=0x{:08X}", static_cast<unsigned int>(envResult)));
					DestroyWindow(hWnd);
					return S_OK;
				}
				...
				return environment->CreateCoreWebView2Controller(
					...
					Microsoft::WRL::Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
						[hWnd](HRESULT controllerResult, ICoreWebView2Controller* controller) -> HRESULT {
							return OnProjectAgentsConfigControllerCreated(hWnd, controllerResult, controller);
						}).Get());
			}).Get());

	if (FAILED(hr)) {
		OutputStringToELog(std::format("[Project AGENTS Config][WebView2] bootstrap failed hr=0x{:08X}", static_cast<unsigned int>(hr)));
		DestroyWindow(hWnd);
	}
}
...
LRESULT CALLBACK ProjectAgentsConfigWebViewDialogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	auto* ctx = reinterpret_cast<ProjectAgentsConfigWebViewDialogContext*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
	switch (uMsg)
	{
	case WM_NCCREATE: {
	...
	case WM_CREATE:
		...
		ctx->hLoading = CreateWindowExW(
			0, L"STATIC", L"正在初始化 WebView2 AGENTS.md 设置页...",
			WS_CHILD | WS_VISIBLE,
			0, 0, 0, 0, hWnd, nullptr, nullptr, nullptr);
		SetDefaultFontLocal(ctx->hLoading);
		LayoutProjectAgentsConfigWebViewDialog(hWnd, ctx);
		SetTimer(hWnd, kProjectAgentsConfigWebViewInitTimerId, kProjectAgentsConfigWebViewInitTimeoutMs, nullptr);
		StartProjectAgentsConfigWebView(hWnd, ctx);
		return 0;
	...
	case WM_TIMER:
		if (ctx != nullptr && wParam == kProjectAgentsConfigWebViewInitTimerId) {
			if (!ctx->webViewReady) {
				OutputStringToELog("[Project AGENTS Config][WebView2] initialization timed out");
				DestroyWindow(hWnd);
				return 0;
			}
			KillTimer(hWnd, kProjectAgentsConfigWebViewInitTimerId);
...
void ShowProjectAgentsConfigDialog(HWND owner)
{
	...
	if (sourcePathText.empty()) {
		MessageBoxW(owner,
			L"当前没有打开易语言源文件，无法确定对应的 AGENTS.md 文件。",
			L"AutoLinker 当前程序 AGENTS.md 设置",
			MB_OK | MB_ICONWARNING);
		return;
	}

	std::filesystem::path sourcePath(sourcePathText);
	if (!IsProjectSourceExtension(sourcePath)) {
		MessageBoxW(owner,
			L"当前打开的文件不是 .e 或 .ec 源文件，无法确定对应的 AGENTS.md 文件。",
			L"AutoLinker 当前程序 AGENTS.md 设置",
			MB_OK | MB_ICONWARNING);
		return;
	}
	...
	ProjectAgentsConfigWebViewDialogContext ctx = {};
	...
	HWND hDialog = CreateWindowExA(
		...
		wc.lpszClassName,
		"AutoLinker Project AGENTS.md Config",
	...
	if (hDialog == nullptr) {
		OutputStringToELog("[Project AGENTS Config][WebView2] CreateWindowExA failed");
		return;
	}
